...
void Surface::CopyHalfSize( Surface* a_Dst )
{
	...
	Pixel* src = GetBuffer();
	const int w = m_Width / 2, h = m_Height / 2;
	for( int y = 0; y < h; y++, src += m_Width ) for( int x = 0; x < w; x++, src += 2, dst++ )
	{
		Pixel p1 = ((src[0] >> 1) & 0x7f7f7f) + ((src[1] >> 1) & 0x7f7f7f);
		Pixel p2 = ((src[m_Width] >> 1) & 0x7f7f7f) + ((src[m_Width + 1] >> 1) & 0x7f7f7f);
...
void Surface::CopyHalfSize(Surface* a_Dst)
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = GetBuffer();
	const int w = m_Width >> 1, h = m_Height >> 1;
	for (int y = 0; y < h; y++, src += m_Width) {
		for (int x = 0; x < w; x++, src += 2)
		{
			Pixel p1 = ((*src >> 1) & 0x7f7f7f) + ((*(src + 1) >> 1) & 0x7f7f7f);
			Pixel p2 = ((*(src + m_Width) >> 1) & 0x7f7f7f) + ((*(src + (m_Width + 1)) >> 1) & 0x7f7f7f);
			*dst++ = ((p1 >> 1) & 0x7f7f7f) + ((p2 >> 1) & 0x7f7f7f);
		}
	}
}
